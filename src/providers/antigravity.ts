...
/**
 * Antigravity provider — now wraps Google Generative AI (as per updated endpoints),
 * sourcing its API key dynamically from the OAuth credential store as JSON.
 */
export class AntigravityOAuthProvider implements Provider {
    ...
    async generateResponse(messages: Message[], tools?: Tool[], options?: ProviderOptions): Promise<ProviderResponse> {
        ...
        const oauthKeyStr = await store.getApiKey('antigravity');
